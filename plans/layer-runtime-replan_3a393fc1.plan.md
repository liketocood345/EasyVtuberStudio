...
- 不在 EasyVtuberStudio 里继续开发完整互动中控、平台接入或跨项目编排逻辑；这些放到其他独立项目
- 不在 EasyVtuberStudio 里继续开发复杂的外部参数编排逻辑；这里只预留 WebSocket 参数输入边界
...
1. 先读 [E:\\easyvtuberstudio-develop\\face-puppeteer-ui-enhancements-ai-code\\experiments\\puppeteer_load_preview\\character_model_mediapipe_puppeteer_load_preview.py](E:\\easyvtuberstudio-develop\\face-puppeteer-ui-enhancements-ai-code\\experiments\\puppeteer_load_preview\\character_model_mediapipe_puppeteer_load_preview.py) 中的：
...
主要变更文件：
- [E:\easyvtuberstudio-develop\face-puppeteer-ui-enhancements-ai-code\experiments\puppeteer_load_preview\character_model_mediapipe_puppeteer_load_preview.py](E:\easyvtuberstudio-develop\face-puppeteer-ui-enhancements-ai-code\experiments\puppeteer_load_preview\character_model_mediapipe_puppeteer_load_preview.py)
- [E:\easyvtuberstudio-develop\face-puppeteer-ui-enhancements-ai-code\experiments\puppeteer_load_preview\README.txt](E:\easyvtuberstudio-develop\face-puppeteer-ui-enhancements-ai-code\experiments\puppeteer_load_preview\README.txt)
...
- 接口默认命名规则：
  ...
  - 组选中优先使用组名
  - 命名冲突时再做稳定去重
- EasyVtuberStudio 主程序只负责生成接口定义和预留本地 WebSocket 接收边界，不在 EasyVtuberStudio 里继续实现完整互动程序

模型参数外部接口预留规则：
- 在“后处理和其他”区域增加一个开关，用于决定是否启用“传入模型参数的外部接口”
- 该接口同样采用 WebSocket
- 接口名直接采用模型参数的 value 名字，不额外做素材/组名式命名
- EasyVtuberStudio 主程序只负责：
  - 预留本地 WebSocket 接收边界
  - 把收到的参数值映射到模型参数输入
- 不在 EasyVtuberStudio 里继续实现：
  - 直播平台事件接入
  - 外部参数编排规则
  - 跨项目中控逻辑
