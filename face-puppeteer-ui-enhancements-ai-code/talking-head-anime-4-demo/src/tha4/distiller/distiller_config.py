...
# THA4_bundle_bai_custom — bai white cat, resume ~310k body.
# Face 0010 is good; pack preview eyes smear to gray -> low color_change, low blended body phases.
BODY_MORPHER_NUM_TRAINING_EXAMPLES_PER_CHECKPOINT = 10_000
BODY_MORPHER_TRAINING_TARGET = 800_000
BODY_MORPHER_EVAL_TARGET_450K = 450_000
BODY_MORPHER_TRAINING_PHASE_UPPER_BOUNDS = (
    450_000,
    650_000,
    BODY_MORPHER_TRAINING_TARGET,
)
BODY_MORPHER_FINAL_CHECKPOINT_INDEX = (
    BODY_MORPHER_TRAINING_TARGET // BODY_MORPHER_NUM_TRAINING_EXAMPLES_PER_CHECKPOINT
)
BODY_MORPHER_CHECKPOINT_INDEX_450K = (
    BODY_MORPHER_EVAL_TARGET_450K // BODY_MORPHER_NUM_TRAINING_EXAMPLES_PER_CHECKPOINT
)


def _bai_custom_body_training_phases() -> TrainingPhases:
    return TrainingPhases([
        TrainingPhase(
            num_examples_upper_bound=BODY_MORPHER_TRAINING_PHASE_UPPER_BOUNDS[0],
            learning_rate=3e-5,
            loss_weights=LossWeights(weights={
                LossTerm.full_blended: 0.5,
                LossTerm.full_warped: 1.5,
                LossTerm.full_grid_change: 2.5,
                LossTerm.full_color_change: 0.4,
            })),
        TrainingPhase(
            num_examples_upper_bound=BODY_MORPHER_TRAINING_PHASE_UPPER_BOUNDS[1],
            learning_rate=1e-5,
            loss_weights=LossWeights(weights={
                LossTerm.full_blended: 1.0,
                LossTerm.full_warped: 2.5,
                LossTerm.full_grid_change: 4.0,
                LossTerm.full_color_change: 0.5,
            })),
        TrainingPhase(
            num_examples_upper_bound=BODY_MORPHER_TRAINING_PHASE_UPPER_BOUNDS[2],
            learning_rate=1e-5,
            loss_weights=LossWeights(weights={
                LossTerm.full_blended: 3.0,
                LossTerm.full_warped: 1.5,
                LossTerm.full_grid_change: 2.0,
...
class DistillerConfig:
    ...
    def get_body_morpher_trainer(self, world_size: Optional[int] = None, backend: str = 'gloo'):
        ...
        args = SirenMorpher03TrainerArgs(
            ...
            num_training_examples_per_checkpoint=BODY_MORPHER_NUM_TRAINING_EXAMPLES_PER_CHECKPOINT,
            training_phases=_bai_custom_body_training_phases())
    ...
    def define_tasks(self, workspace: Workspace):
        ...
        body_ckpt = f"{self.body_morpher_prefix()}/checkpoint/{BODY_MORPHER_FINAL_CHECKPOINT_INDEX:04d}/module_module.pt"
        @file_task(workspace, self.character_model_body_morpher_file_name(), [
            body_ckpt,
        ])
        def copy_body_morpher():
            copy_file(
                body_ckpt,
