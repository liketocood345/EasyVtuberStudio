...
顶栏：**以 THA3 加载上次/其他立绘**、**加载 THA4 Student 模型** 等。图像来源由 `image_source_mode` 与加载动作决定（无后处理区单选）。

## 持久化 (`load_preview_ui_state.json`)

路径一律以 **fork 仓库根** 为基准的相对路径（正斜杠），示例：
...
读写由 `tha3_paths.to_repo_relative` / `from_repo_relative` 自动转换，勿手写 `E:\...` 绝对路径。

## 依赖（分黑盒配置）

清单与脚本在 **`deps/pip/`**（相对仓库根）：
