...
class MainFrame(wx.Frame):
    ...
    def sync_output_frame_owner(self):
        ...
        if getattr(self, "output_frame", None) is None:
            return
        self.output_frame.SetOwner(None)
    ...
    def create_controls_frame(self):
        ...
        try:
            ...
            self.load_last_model_button = wx.Button(
                controls_header_panel, wx.ID_ANY, "加载上次模型 / Load Last Model")
            ...
            self.load_model_button = wx.Button(
                controls_header_panel, wx.ID_ANY, "加载其他模型 / Load Another Model")
    ...
    def update_result_image_bitmap(self, event: Optional[wx.Event] = None):
        ...
        if getattr(self.pose_converter, "refresh_audio_input_runtime", None) is not None:
            # Before a model is loaded, skip high-frequency audio UI refresh to avoid
            # relayout churn on the model-input column that breaks hover-to-wheel arming.
            if self.is_model_loaded() and (self.poser is None or self.mediapipe_face_pose is None):
                self.pose_converter.refresh_audio_input_runtime(time.time())
        if self.poser is None:
            self.initialize_output_bitmap()
            dc = wx.MemoryDC()
