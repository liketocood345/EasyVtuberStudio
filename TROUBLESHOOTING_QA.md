...
基于 Load Preview 实验增强、camfix 隔离测试与排障经历整理。  
**文档权威副本（本文件）：** `E:\tha4fork\` · **活跃代码开发：** `E:\tha4fork-develop\`（稳定后合并到 fork 再 push）。

相关文档：[HARDWARE_REQUIREMENTS.md](HARDWARE_REQUIREMENTS.md) · [BACKUP.md](BACKUP.md) · [CHANGELOG.md](CHANGELOG.md) · [docs/DOC_INDEX.md](docs/DOC_INDEX.md)
...
| 常见目标 | 含义 |
|----------|------|
| `E:\tha4fork\...\run_load_preview_puppeteer.bat` 或 `》》》》start《《《《.bat` | **发布总库**（定制客户部署、GitHub 推送） |
| `E:\tha4fork-develop\...\run_load_preview_puppeteer.bat` | **研发主仓**（日常改代码） |
...
`face-puppeteer-ui-enhancements-ai-code\talking-head-anime-4-demo\venv\Scripts\python.exe`

`PYTHONPATH` 指向该 demo 的 `src`。依赖安装见 `deps\pip\install_all_image_source_deps.bat`。
...
**A：** **`E:\tha4fork-develop` = 活跃研发**；**`E:\tha4fork` = 对外发布总库**（定制部署、GitHub 文档、push）。Markdown 说明以 **fork 根目录** 为准（本文件、`HANDOVER.md`、`docs/DOC_INDEX.md`）；代码以你实际运行的 bat 所在目录为准。
...
**A：** 在开启 **「启用自动移动缩放」** 时，显示层会按 **Smoothing** 做**平滑过渡**（含左右归中与缩放基准更新），**不是 bug**。若需立即到位，可暂时关闭自动移动缩放再校准，或等待数秒观察。定制培训中说明一次即可，**无需反复点击校准**。
...
```text
1. 确认运行的 bat / 脚本路径（fork 发布：E:\tha4fork\》》》》start《《《《.bat）
...
*文档版本：2026-05-29 · 含窗口捕获、DroidCam 绕行、定制化持久化预期（第九节）。活跃代码在 `E:\tha4fork-develop`；**本文档以 fork 根目录为权威副本**。*
