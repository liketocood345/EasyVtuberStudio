...
本仓库 **`face-puppeteer-ui-enhancements-ai-code/`** 内用 **`his/` = 只读历史快照** 留档。  
...
fork **根目录**的 `README.md`、`TROUBLESHOOTING_QA.md`、`HANDOVER.md` 等不在 `his/` 快照内，需在 **`E:\tha4fork\`** 根单独维护。
...
- 仓库根 **`》》》》start《《《《.bat`** 转发到 `run_load_preview_puppeteer.bat`
